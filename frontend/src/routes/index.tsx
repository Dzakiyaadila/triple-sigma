  ...
  head: () => ({
    meta: [
      { title: "Pilih Data — RestockIQ" },
      {
        name: "description",
        content:
          "Mulai run restock dengan data simulasi bawaan atau unggah data transaksi toko sendiri.",
      },
      { property: "og:title", content: "Pilih Data — RestockIQ" },
      {
        property: "og:description",
        content: "Langkah pertama: pilih sumber data dan periksa laporan kesiapan data.",
      },
    ],
  }),
  component: PilihData,
...
function PilihData() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <SectionTitle
        title="Mulai dengan data toko"
        desc="Gunakan data simulasi atau unggah data toko kamu sendiri"
      />

      <div className="grid gap-4 md:grid-cols-2">
        <div
          className={cn(
            "relative rounded-[6px] border bg-card p-5",
            dataset?.kind === "demo" ? "border-accent-gold" : "border-border",
          )}
        >
        ...
        <div
          ...
          className={cn(
            "rounded-[6px] border bg-card p-5",
            dataset?.kind === "upload" ? "border-accent-gold" : "border-border",
          )}
        >
          ...
          <button
            ...
            onClick={() => {
              const header =
                "date,store_id,sku_id,units_sold,stock_on_hand_start,stock_on_hand_end,stockout_flag,promo_flag";
      ...
      {validation === "done" && dataset ? (
        <div className="mt-6 rounded-[6px] border border-border bg-card p-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <GhostButton className="ml-auto h-8" onClick={resetDataset}>
              Ganti sumber data
            </GhostButton>
          </div>
            ...
            {[
              ...
              <div key={label as string} className="rounded-[6px] border border-border p-3">
                ...
                <dd className="num mt-1 text-lg font-semibold">
                  {(val as number).toLocaleString("id-ID")}
                </dd>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            {dataset.hasFatal ? (
              ...
              <p className="flex items-center gap-2 text-xs text-safe">
                <CheckCircle2 className="h-4 w-4" /> Data siap dipakai untuk membuat rencana
                restock.
              </p>
            )}
          </div>
